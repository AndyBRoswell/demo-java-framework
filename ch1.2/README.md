...
Spring 为软件架构中的这些常见的层提供了有关依赖注入的、用于声明 bean 的注解。如：控制（器）层、服务层和数据访问层（持久层），是依次从上层（应用层）到底层（核心层）的几个层次。这几个层常见于众多知名的大型软件。它们对应的注解分别为 @Controller、@Service 和 @Repository。
...
Spring 还提供了用于注入 bean 的注解：被 @Autowired 标注
